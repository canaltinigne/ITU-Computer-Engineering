...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
    ...
    // If robot is not 1.2 meters close to a wall go linearly
    // If it is close turn it to left
    
   if(!std::isnan(laser_ranges[laser_ranges.size()/2 - laser_ranges.size()/4]) && !std::isnan(laser_ranges[laser_ranges.size()/2 + laser_ranges.size()/4]) ){
    if(laser_ranges[laser_ranges.size()/2 - laser_ranges.size()/4] > 1.2 && laser_ranges[laser_ranges.size()/2 + laser_ranges.size()/4] > 1.2){
   ...
   } else {					// If robot is very close to a wall from left and right, This is the Panic Station of TurtleBot !
     motor_command.angular.z = 0.8;
